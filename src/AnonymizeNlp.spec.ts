...
Please remember that all the information provided, including the credit card number, email address, and phone number, is entirely fictional and randomly generated. It does not represent any real individuals or their personal experiences.`;
    const anonymized = anonymizeNlp.anonymize(input);
    expect(anonymized).toEqual(
      `My name is <FIRSTNAME> <LASTNAME>, and I was born on <DATE>, in a small town called <LOCATION>. I grew up with my parents and two siblings, an older brother named <FIRSTNAME> and a younger sister named <FIRSTNAME>. We lived in a cozy two-story house with a white picket fence. In high school, I was actively involved in the drama club and played the lead role in our school's production of "Romeo and Juliet." After graduation, I pursued my passion for writing and earned a Bachelor's degree in English Literature from the University of Cambridge in <DATE>. Currently, I work as a freelance writer, specializing in content creation for various online platforms.`,
    );
    const deAnonymized = anonymizeNlp.deAnonymize(anonymized);
    expect(deAnonymized).toEqual(input);
  });
